...
// the Proselenos project. The key change is the addition of a reload
// functionality for AI tools. A new `handleReloadTools` callback has been
// introduced, and it is passed into `AIToolsSection` via the new
// `onReloadTools` prop. This function calls `toolsActions.loadToolsFromGoogleDrive`
// to refresh tool prompts from the user's Google Drive. All other logic
// remains unchanged.
...
export default function ClientBoot({ init }: { init: InitPayloadForClient | null }) {
  ...
  This is a one‑time setup step. Once granted, you won't see this permission screen again.`;
                ...
                showStickyErrorWithLogout('Google Drive Access Required', detailedPermissionError, isDarkMode);
              } else {
                showStickyErrorWithLogout('Initialization failed', `Tool-prompts install failed: ${errorMsg}`, isDarkMode);
              }
              return;
            }
          } catch (error) {
    ...
    try {
      ...
      if (result.success && result.data?.files) {
        const txtFiles = result.data.files.filter((file: any) =>
          (typeof file.name === 'string' && file.name.toLowerCase().endsWith('.txt')) || file.mimeType === 'text/plain'
        );
  ...
  const handleProjectSettings = async () => {
    ...
    try {
      const result = await loadProjectMetadataAction(
        session.accessToken,
        init?.config?.settings.proselenos_root_folder_id || '',
        projectState.currentProjectId
      );
  ...
  // New handler: reload AI tools
  const handleReloadTools = async () => {
    try {
      // Trigger a reload of the tool-prompts from Google Drive. This will update
      // the available tools and categories behind the scenes.
      await toolsActions.loadToolsFromGoogleDrive(isDarkMode);
      // Once reloading is complete, reset the category and tool selections to
      // their defaults so the user sees the dropdowns cleared. Also clear
      // toolsInCategory to avoid leftover tool options from the previous
      // selection. These actions rely on methods provided by the tools manager.
      toolsActions.setSelectedCategory('');
      toolsActions.setSelectedTool('');
      toolsActions.setToolsInCategory([]);
    } catch (error) {
      console.error('Error reloading AI tools:', error);
    }
  };
  ...
  const handleProjectSettingsSave = async (metadata: ProjectMetadata) => {
    ...
    try {
      const result = await saveProjectMetadataAction(
        session.accessToken,
        init?.config?.settings.proselenos_root_folder_id || '',
        projectState.currentProjectId,
        metadata
