...
export async function executePublishingWithProgress(
  ...
  try {
    ...
    const result = await publishManuscriptAction(manuscriptFileId, projectFolderId);
...
export async function generateIndividualFile(
  ...
  try {
    ...
    switch (fileType) {
      case 'html':
        result = await generateHTMLOnlyAction(manuscriptFileId, projectFolderId);
        break;
      case 'epub':
        result = await generateEPUBOnlyAction(manuscriptFileId, projectFolderId);
        break;
      case 'pdf':
        result = await generatePDFOnlyAction(manuscriptFileId, projectFolderId);
        break;
      default:
