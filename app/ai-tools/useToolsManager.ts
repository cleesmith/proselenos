...
import { listGoogleDriveFilesAction, readGoogleDriveFileAction } from '@/lib/google-drive-actions';
...
interface ToolsManagerActions {
  ...
  loadToolsFromGoogleDrive: (isDarkMode: boolean) => Promise<void>;
  
  // File selection
  setupAITool: (session: any, rootFolderId: string, currentProject: string | null, currentProjectId: string | null, setIsGoogleDriveOperationPending: (loading: boolean) => void, isDarkMode: boolean) => Promise<void>;
...
export function useToolsManager(): [ToolsManagerState, ToolsManagerActions] {
  ...
  // Load tools from Google Drive tool-prompts folder (fast, assumes tools are already initialized)
  const loadToolsFromGoogleDrive = useCallback(async (isDarkMode: boolean) => {
    try {
      ...
      if (result.success && result.tools) {
        ...
        console.error('Failed to load tools from Google Drive:', result.error);
        showAlert(`Failed to load tools: ${result.error}`, 'error', undefined, isDarkMode);
        setToolsReady(false);
      }
    } catch (error) {
      console.error('Error loading tools from Google Drive:', error);
  ...
  const executeAITool = useCallback(async (
    ...
    try {
      ...
      const fileResult = await readGoogleDriveFileAction(session.accessToken, rootFolderId, selectedManuscriptForTool.id);
      ...
      if (toolExecResult.success && toolExecResult.result) {
        ...
        setUploadStatus('Saving report to Google Drive...');
