...
export default function AIToolsSection({
  ...
  // Dual panel view state (existing - keep as is)
  const [showDualPanel, setShowDualPanel] = useState(false);
  const [viewManuscriptContent, setViewManuscriptContent] = useState('');
  ...
  const formatFullReport = (toolResult: string, toolId: string, currentProvider: string, currentModel: string, manuscriptFileName: string, currentProject: string) => {
    ...
    const manuscriptPath = `My Drive › storygrind_projects › ${currentProject} › ${manuscriptFileName}`;
    
    // Format the report content exactly like the original Node.js app with manuscript file info
    return `=== ${displayToolName.toUpperCase()} REPORT ===
Date: ${dateTimeStr}
...
---
Report generated by StoryGrind
...
`;
  ...
  const handleViewClick = async () => {
    if (selectedManuscriptForTool) {
      // Load manuscript content for dual panel display
      try {
        // Import the action here to avoid potential bundling issues
        const { readGoogleDriveFileAction } = await import('@/lib/google-drive-actions');
        ...
        if (fileResult.success && fileResult.data?.content) {
          setViewManuscriptContent(fileResult.data.content);
          setShowDualPanel(true);
        } else {
          setViewManuscriptContent('Failed to load manuscript content.');
          setShowDualPanel(true);
        }
      } catch (error) {
        setViewManuscriptContent('Error loading manuscript content.');
        setShowDualPanel(true);
      }
    }
  };
  ...
  return (
    <div style={{ 
      ...
      {/* Dual Panel View Modal (existing - keep as is) */}
      <DualPanelView
        isVisible={showDualPanel}
        onClose={() => setShowDualPanel(false)}
        manuscriptContent={viewManuscriptContent}
