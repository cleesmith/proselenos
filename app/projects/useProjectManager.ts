...
export function useProjectManager(): [ProjectManagerState, ProjectManagerActions] {
  ...
  const openProjectSelector = useCallback(
    async (session: any, rootFolderId: string, isDarkMode: boolean) => {
  ...
  const selectProject = useCallback(
    async (
      ...
      // Save to config
      try {
        const result = await selectProjectAction(
          session.accessToken,
          rootFolderId,
          folder.name,
          folder.id
  ...
  const createNewProject = useCallback(
    async (
      ...
      try {
        const result = await createProjectFolderAction(
          session.accessToken,
          rootFolderId,
          newProjectName.trim()
        );
      ...
      try {
        ...
        if (result.success && result.data?.files) {
          if (result.data.files.length === 0) {
            showAlert(
              'No DOCX files found in the current project. Please upload a DOCX file to your project folder first.',
              'info',
              undefined,
              isDarkMode
            );
        ...
        showAlert(
          `Error loading DOCX files: ${error instanceof Error ? error.message : String(error)}`,
          'error',
          undefined,
          isDarkMode
        );
  ...
  const performDocxConversion = useCallback(
    ...
    [selectedDocxFile, outputFileName, currentProjectId]
  );
    ...
    async (
      ...
      try {
        ...
        if (result.success && result.data?.files) {
          if (result.data.files.length === 0) {
            showAlert(
              'No TXT files found in the current project. Please add a TXT file to your project folder first.',
              'info',
              undefined,
              isDarkMode
            );
        ...
        showAlert(
          `Error loading TXT files: ${error instanceof Error ? error.message : String(error)}`,
          'error',
          undefined,
          isDarkMode
        );
  ...
  const performTxtConversion = useCallback(
    ...
    [selectedTxtFile, txtOutputFileName, currentProjectId]
  );
