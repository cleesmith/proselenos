  ...
  const handleFileClick = async (file: any) => {
    ...
    if (isFolder) {
      ...
      try {
        ...
        const result = await readGoogleDriveFileAction(session.accessToken, rootFolderId, file.id);
        if (result.success) {
          onLoadFileIntoEditor(result.data?.content, file.name, file.id);
          onClose();
    ...
    <div style={{
      ...
      <div style={{
        ...
        {isProjectFilesBrowser ? (
                ...
                return (
                  ...
                  >
                    <div style={{ 
                      ...
                      {isFolder ? <span>📁</span> : (isDoc ? <span>📝</span> : <span>📄</span>)}
                      {file.name}
                    </div>
                    <div style={{ 
                      fontSize: '10px',
          ...
          modalFiles.filter(file => file.mimeType === 'application/vnd.google-apps.folder' && file.name !== 'tool-prompts').length === 0 ? (
            ...
            <div style={{ 
              ...
              {modalFiles
                .filter(file => file.mimeType === 'application/vnd.google-apps.folder' && file.name !== 'tool-prompts')
                .map((folder: any) => (
                  <div
                    key={folder.id}
